...
//
// In-process end-to-end echo test for the grpcudspp wrapper.
//
//   ServerBuilder + RegisterService(EchoService) → BuildAndStart
//   nghttp2 client over a real AF_UNIX socket
//   single-threaded pump alternating client mem_send / server tick / read
//
// Mirrors rust/grpcuds-ffi-impl/tests/echo.rs but on the C++ side, so we
// verify that the ServerBuilder pattern compiles, links, and runs against
// the live cdylib/staticlib output.
...
// Release-safe assert: the round trip below is built from side-effecting
// calls (nghttp2 setup, submit, mem_send/recv, tick). `assert` would strip
// all of them under NDEBUG, leaving null state + a crash, so use a macro
// that always evaluates its argument and bails the test on failure.
#define CHECK(x)                                                         \
    do {                                                                 \
        if (!(x)) {                                                      \
...
// ---- The user's service, hand-rolled (no plugin needed) ---------
...
class EchoService : public grpcuds::Service {
 ...
 private:
    static int Trampoline(void* call, int32_t call_id, const uint8_t* req,
        ...
        auto* self = static_cast<EchoService*>(user_data);
...
// ---- Client-side nghttp2 plumbing (mirrors the Rust test) -----------------

struct ClientState {
    std::vector<uint8_t> status_value;
    std::vector<uint8_t> content_type_value;
...
int on_header_cb(nghttp2_session*, const nghttp2_frame*, const uint8_t* name,
    ...
    } else if (namelen == 12 && std::memcmp(name, "content-type", 12) == 0) {
        st->content_type_value.assign(value, value + valuelen);
...
std::string unique_path() {
    ...
    char buf[64];
    std::snprintf(buf, sizeof(buf), "/tmp/grpcudspp-echo-%d-%d.sock", getpid(),
                  counter++);
...
int main() {
    ...
    for (int loop = 0; loop < 128; ++loop) {
        ...
        // Server tick via the C ABI directly (no need for a special wrapper).
    ...
    CHECK(check(cli_state.status_value, "200"));
    CHECK(check(cli_state.content_type_value, "application/grpc"));
    CHECK(check(cli_state.grpc_status_value, "0"));
