...
//
// End-to-end test for grpcuds::ServerThread.
//
// Unlike echo_test.cc (which hand-pumps Accept()/tick() on the test thread),
// here the server runs entirely on ServerThread's own background thread: it
// accepts the connection and drives the nghttp2 session itself. The test
// thread only plays the gRPC client over a real AF_UNIX socket. This proves
// the "BuildAndStart + run in the background, main thread stays free" model.
...
std::string unique_path() {
    ...
    char buf[64];
    std::snprintf(buf, sizeof(buf), "/tmp/grpcudspp-st-%d-%d.sock", getpid(),
                  counter++);
...
int main() {
    ...
    // ---- Server runs on its own background thread ----
    ...
    CHECK(server_thread.running());

    // ---- Client connects (socket is already bound) ----
    ...
    for (int i = 0; i < 100 && crc != 0; ++i) {
        crc = ::connect(client_fd, reinterpret_cast<sockaddr*>(&addr),
                        sizeof(addr));
        if (crc != 0) std::this_thread::sleep_for(std::chrono::milliseconds(1));
    }
    CHECK(crc == 0);
    ...
    // ---- Drive the client; the server thread reacts on its own ----
    for (int loop = 0; loop < 2000 && !cli_state.end_stream_seen; ++loop) {
        for (;;) {
    ...
    // ---- Stop the server thread (destructor would also do this) ----
    server_thread.Stop();
    CHECK(!server_thread.running());
    ...
    std::printf("server_thread round-trip OK\n");
    return 0;
}
